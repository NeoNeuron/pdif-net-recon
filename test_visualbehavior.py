...
for out_dir in Path('./visualbehavior/').iterdir():
    ...
    for stimulus_, gap_, axi in zip(stimulus_names_plot, gap_vals, ax.flatten()):
        ...
        vrange=(-8,-2)
        data_recon, data_fig = reconstruction_analysis(data_matched, nbins=200, hist_range=vrange, fit_p0=[0.5,-6.5,-4.2,2,1])
    ...
    for key in ('CC', 'MI', 'GC', 'TE'):
        ...
        for stim, axi in zip(stimulus_names_plot, ax.flatten()):
            vrange=(-8,-2)
            counts, bins = np.histogram(selected_data[selected_data['stimulus'].eq(stim)]['log-'+key], bins=200, range=vrange, density=True)
