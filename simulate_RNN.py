...
@njit
def evolve_RNN(noise:np.ndarray, W:np.ndarray, 
        x_init:np.ndarray=None, tau:float=20., dt:float=1., 
    ):

    """Evolve Recurrent Neural Network model with sparse connectivity, with tanh nonlinearity.
    ...
    state = np.zeros(num_units) if x_init is None else x_init
    alpha = dt/tau
    sigma = 1/np.sqrt(dt)
    for i in range(num_len):
        signal[i] = (1-alpha)*state + alpha*(W@np.tanh(state) + sigma*noise[i])
        state = signal[i].copy()
    return signal

def sim_RNN(data_path, N:int=2,
    ...
    fname = f"RNNp={p:.2f}s={s:.3f}tau={tau:.0f}ref={int(ref):d}"
    ...
    if force_regen or not spk_fname.exists():
        ...
        for start, period in tranges:
            if gen_voltage_flag:
                ...
                x = evolve_RNN(noise, W*s, x0, tau, dt)
                x0 = x[-1, :].copy()
...
if __name__ == '__main__':
    ...
    spk_fname, vol_fname = sim_RNN(**pm_dym, force_regen=False)
