#!/bin/bash
# Run from the repo root: ./slurms/batch_bm_CCM.sh
...
# array in batch_bm_PDIF.sh / batch_bm_DDC.sh / batch_bm_STE.sh / batch_bm_GLMCC.sh.
    ...
    do
        ...
        do
            ...
            do
                ...
                outfile="results/${ccm}/recon_df_noise_${noise_tag}_${keys[i]}_T=${T_tag}_${idx}.pkl"
                if [ -f "$outfile" ]; then
                    echo "Skip (exists): $outfile"
                    continue
                fi
                sbatch --export=ALL,CCM=${ccm},KEY=${keys[i]},IDX=${idx},NOISE_LEVEL=${noise_level},T=${Ts[i]} slurms/bm_CCM.slurm
            done
        done
    done
done