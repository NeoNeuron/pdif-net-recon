...
from pdif.utils import match_features, reconstruction_analysis, optimal_delay_estimator
from pdif.figrc import line_rc, c_inv, fig_path
...
for out_dir in Path('./visualcoding/').iterdir():
    ...
    #! ====================
    stimulus_names_plot = ['drifting_gratings', 'static_gratings', 'natural_scenes', 'natural_movie']
    ...
    #     # data['delay'] = delay_
    #     # tmp.append(data)
    #     tmp.append(data['sum(MI)'].values)
    # data = np.asarray(tmp)
    # data = pd.concat(tmp)
    #%%
    # max_ids = np.argmax(data, axis=0)
    # plt.hist(max_ids, bins=21)
    #%%
    # sns.pointplot(data=data[(data['pre_id']==0)*(data['post_id']==1)], x='delay', y='TE', errorbar='sd')
    # sns.pointplot(data=data[(data['pre_id']==1)*(data['post_id']==0)], x='delay', y='TE', errorbar='sd')
    ...
    for stimulus_, gap_, axi in zip(stimulus_names_plot, gap_vals, ax.flatten()):
        ...
        icon_name = 'natural_movie.png' if 'natural_movie' in stimulus_ else stimulus_+'.png' 
        arr_image = plt.imread(fig_path / icon_name, format='png')
        axins = inset_axes(axi, width="100%", height="100%",
                        bbox_to_anchor=(.1, .4, .23, .23),
    ...
    for key in ('CC', 'MI', 'GC', 'TE'):
        print(f"{key:s}: {tmp['recon-gauss-'+key].isin([2]).mean()*100:>5.2f} %")
    #%%

    inconsist_hist = {key:{} for key in stimulus_names_plot}
    for key in ('CC', 'MI', 'GC', 'TE'):
        mask = tmp['recon-gauss-'+key].isin([2])
        ...
        for stim, axi in zip(stimulus_names_plot, ax.flatten()):
            ...
            pop_ratio = tmp['recon-gauss-'+key].isin([2]).mean()
            ...
            axi.set_ylim(0)
            icon_name = 'natural_movie.png' if 'natural_movie' in stim else stim+'.png' 
            arr_image = plt.imread(fig_path / icon_name, format='png')
            axins = inset_axes(axi, width="100%", height="100%",
    ...
    for idx, key in enumerate(('CC', 'MI', 'GC', 'TE')):
        ...
        # draw x-axis
        for i, stim in enumerate(stimulus_names_plot):
            icon_name = 'natural_movie.png' if 'natural_movie' in stim else stim+'.png' 
            arr_image = plt.imread(fig_path / icon_name, format='png')
    ...
    for i, axi in enumerate(ax.flatten()[:len(stimulus_names_plot)]):
        ...
        icon_name = 'natural_movie.png' if 'natural_movie' in stimulus_names_plot[i] else stimulus_names_plot[i]+'.png' 
        arr_image = plt.imread(fig_path / icon_name, format='png')
        axins = inset_axes(axi, width="100%", height="100%",
