...
data_matched = c4u.match_features(data, N=100,
                                    conn_file=root/f'HH100/connect_matrix-p=0.250.dat')
data_recon, fig_data = c4u.reconstruction_analysis(data_matched, nbins=100)
fig = mplt.reconstruction_illustration(fig_data)
#%%
TE = data['TE'].to_numpy().reshape(100,100)
conn = np.fromfile(root/'HH100/connect_matrix-p=0.250.dat', dtype=float).reshape(100,100)
# %%
...
for T in Ts:
    ...
    data_matched = c4u.match_features(
        data, N=100, conn_file=root/f'HH100/connect_matrix-p=0.250.dat')
...
#%%
fig, ax = plt.subplots(1,1,figsize=(6,5))
ax.plot(Ts, auc, 'o-', clip_on=False, mec='w', ms=10)
...
fig.savefig(root/'fig_nc/pdf/HH100_TE_vs_length.pdf', bbox_inches='tight')