...
int main(int argc,char **argv) {
  ...
  config.add_options()
    ...
    ("S",           po::value<string>()->default_value("0.02 0.02 0.02 0.02"), "Synaptic coupling strength, E2E, E2I, I2E, I2I.")
    ...
    ("full_mode",   po::value<int>()->default_value(0), "Connectivity init mode: 0=random Erdos-Renyi (P_c/random_S), 1=full CLI entry via --conn_matrix, 2=load weighted matrix from --conn_matrix_file. Also governs how --f is used, see --f help.")
    ("f",           po::value<string>()->default_value(""), "FFWD Poisson strength, one value per neuron, separated by space. If full_mode=1, must have at least N=NE+NI entries (error otherwise). If full_mode=2 and fewer than N entries are given (e.g. left at its default), falls back to homogeneous fE/fI instead. Ignored if full_mode=0.")
    ("fE",          po::value<double>()->default_value(0.1), "Homogeneous FFWD Poisson strength for E neuron, if full_mode=0, or full_mode=2 without a full --f list.")
    ("fI",          po::value<double>()->default_value(0.1), "Homogeneous FFWD Poisson strength for I neuron, if full_mode=0, or full_mode=2 without a full --f list.")
    ("conn_matrix", po::value<string>()->default_value(""), "row-wise connectivity matrix, separated by space. Used when full_mode=1.")
    ("conn_matrix_file", po::value<string>()->default_value(""), "path to a weighted (N,N) .npy connectivity matrix. Used when full_mode=2: row=presynaptic, column=postsynaptic; nonzero entries become edges and their value becomes CS directly (random_S is ignored).")
    ...
    ("record_IE",   po::value<int>()->default_value(0), "toggle to record per-neuron excitatory input current after each T_step.")
    ("record_II",   po::value<int>()->default_value(0), "toggle to record per-neuron inhibitory input current after each T_step.")
    ("record_Ilim", po::value<string>()->default_value("0 1e8"), "time range to record E/I input current, independent of record_vlim.")
